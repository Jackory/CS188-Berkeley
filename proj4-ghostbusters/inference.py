...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        if (jailPosition==ghostPosition and noisyDistance == None):
            return 1
        elif(jailPosition==ghostPosition or noisyDistance == None):
            return 0 
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.allPositions:
            ...
            tmp[ghostPos] = prob * self.beliefs[ghostPos] 
        self.beliefs = tmp 
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for k in self.particles: 
            weight[k] += self.getObservationProb(observation, gameState.getPacmanPosition(),k,self.getJailPosition())
        if(weight.total() == 0):
            self.initializeUniformly(gameState) 
        else:
            weight.normalize()
            self.particles = [weight.sample() for _ in range(self.numParticles)] 
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        tmp = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(tmp)
        self.particles = tmp 
