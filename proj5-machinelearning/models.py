...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        xb1 = nn.AddBias(x1,self.b1)
        xy1 = nn.ReLU(xb1)
        x2 = nn.Linear(xy1, self.w2)
        xy2 = nn.AddBias(x2, self.b2)
        return xy2
...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        x3 = nn.AddBias(nn.Linear(x2, self.w3), self.b3)
        return x3
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        """
        "*** YOUR CODE HERE ***"
        z0 = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.w1),self.b1))
        h = nn.AddBias(nn.Linear(z0, self.w2),self.b2)
        for x in xs[1:]:
            z1 = nn.Add(nn.Linear(x,self.w1), nn.Linear(h,self.w2))
